...
export default function ShareButton() {
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      ...
      // Fallback: Copy to clipboard if native share is not available
      try {
    ...
    // 2. Ouvrir la modale QR Code dans tous les cas (Double Action)
    setIsModalOpen(true);
  };
      ...
      <AnimatePresence>
        {isModalOpen && (
          <div className="fixed inset-0 z-[10000] flex items-center justify-center p-6 lg:hidden">
            ...
            >
              <div className="mb-6 w-16 h-16 rounded-2xl bg-primary-gold/10 flex items-center justify-center">
                <QrCode className="text-primary-gold" size={32} />
              </div>

              <h3 className="text-surface text-2xl font-bold mb-2">Partager le Dojo</h3>
              <p className="text-slate-500 text-sm mb-8">Faites scanner ce code pour transmettre le lien.</p>

              {/* QR Code Image */}
              <div className="bg-white p-4 rounded-2xl border-2 border-slate-100 mb-8 shadow-inner">
                <img 
                  src={qrCodeUrl} 
                  alt="QR Code du Dojo" 
              ...
              <button
                onClick={() => setIsModalOpen(false)}
                className="w-full bg-surface text-white font-bold py-4 rounded-2xl hover:bg-surface-high transition-colors flex items-center justify-center gap-2"
              >
                <X size={20} />
                Fermer
              </button>
